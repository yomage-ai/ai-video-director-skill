# AI 视频导演 Skill

简体中文 | **[English](README.md)**

把下面这段话复制给 Codex 或其他 Coding Agent。附上录制视频或提供本地路径；文稿和风格参考可选：

```text
优先使用我已安装的本地 $ai-video-director Skill。如果没有安装，再读取并使用这个仓库：
https://github.com/yomage-ai/ai-video-director-skill

我提供的素材：[视频附件或本地路径；有文稿和参考时一并提供]
目标平台与画幅：[例如：抖音，9:16]
这条视频想表达的重点：[填写一句话；不确定时请先帮我提炼]
风格要求或参考：[可选；默认使用内置小熊风格]

请先检查素材，用中文说明视频重点、保留与删减的内容，并给我粗剪方案。
等我确认后再继续制作。
粗剪锁定后，先给我一个 6-12 秒、包含真实对白和声音设计的精剪样片，再制作完整精剪。
完成后交付成片、可编辑工程和检查记录。
如果需要发布，再提供多套封面标题、发布文案、标签和 AI 声明方案。
沿用本任务已有授权。只有确实新增付费、登录、系统权限、身份或声音使用、素材权利授权时，再请我处理。
```

如果只想做粗剪：

```text
使用 $ai-video-director 处理这段录制视频，只做粗剪：删除口误、重录和多余等待，保留完整发音与自然节奏。先给我内容与粗剪方案；粗剪通过后，交付粗剪视频、可编辑时间线工程和字幕，到此结束。
```

本 Skill 将录制好的真人口播剪成视频，支持去除口误和重录、制作字幕、调整声音与画面，以及添加合适的动画、音乐和音效。你可以只做粗剪，也可以继续制作完整精剪。

默认内置小熊风格，涵盖字幕、对白音量、自然提亮、配色和画面布局。你也可以提供自己的风格参考，或让 Agent 根据内容推荐方向。需要小熊角色形象或专属片尾时，另需安装并取得 `xiaoxiong-ip` 的使用授权；剪辑自己的视频无需安装它。

首次使用时，Agent 会检查并准备所需工具。遇到必须由你完成的登录、系统权限或付费确认时，Agent 会说明具体步骤；新安装的插件可能需要开启新任务后才能使用。

## 用户需要提供

- 真人口播视频，也支持屏幕与摄像头已经录成一张画面的素材。文稿有帮助，但不能代替录制素材。
- 目标平台、画幅和视频用途。
- 必须保留的观点、内容、来源或参考素材。
- 已有的风格偏好或视觉参考（可选）。
- 续剪已有视频时，提供原片和可编辑工程。

## 你会得到什么

- **粗剪方案**：先确认视频重点、内容取舍和顺序，再开始剪辑。
- **视听样片**：粗剪确认后，先看一段 6–12 秒的精剪效果，包含真实对白、字幕和声音设计；满意后再制作完整视频。
- **成片与工程**：最终视频、可编辑工程和检查记录。只做粗剪时，交付粗剪视频、可编辑时间线和字幕。
- **发布文案**：需要发布时，提供多套封面标题、文案、标签和 AI 声明方案供你选择。

素材检查、转写、工具准备、风格推荐、渲染和交付检查由 Agent 完成。你只需提供素材与目标，并确认内容取舍和样片效果。

## 开源协议

本仓库原创的代码、文档、脚本和模板采用 [Apache License 2.0](LICENSE) 开源。第三方工具、模型、字体、服务和用户提供的素材继续遵循各自的协议与权利条件。
