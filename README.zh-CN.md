...
首次使用时，Agent 会检查并准备所需工具。遇到必须由你完成的登录、系统权限或付费确认时，Agent 会说明具体步骤。新安装的插件可能需要新任务才能使用。

完整粗剪验收还需要宿主提供真实声音审查能力，本 Skill 没有附带听审模型。Agent 会先实测；无法接收声音时，会明确说明限制，不把未听审视频作为完成结果。

已经自己剪过一版？可以直接说：“保留我选的片段、顺序、倍速和声音，只优化指定画面。”Agent 会锁定这版母版，补充缺失的画面、字幕、顶部进度，并检查修改范围；改声音或内容结构仍走正常验收。已有有效批准直接复用。
