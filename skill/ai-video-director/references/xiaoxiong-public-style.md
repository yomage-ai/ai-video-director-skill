...
For screen-led landscape work, `talkingHead.landscapeScreen` / `xiaoxiong-landscape-screen-v1` overrides portrait caption coordinates and any notebook caption background: native approved canvas, **top progress**, transparent horizontally centered **topmost bottom captions**, original screen-video motion and muted supporting footage. Presenter cutout is optional and uses the approved normalized size; do not turn one “shrink by half” correction into a universal size. Preserve approved transitions; an abandoned experiment is not a permanent ban on transitions. / 横屏录屏主导时，上述专用预设优先于竖屏坐标和手账字幕底色：保留原画布、顶部进度、底部水平居中无底色顶层字幕。人物仅补充缺失内容；抠像与大小沿用已确认参考，排除椅子并检查动态边缘。用户本片撤回一种转场，不等于所有视频禁用转场。
