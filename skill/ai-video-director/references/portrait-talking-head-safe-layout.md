...
`layout.portrait-talking-head.safe-v1` is the approved repository-level reference for a 9:16 talking-head composition that combines captions, semantic chapter progress, B-roll, picture-in-picture, and platform UI safety. It is non-personal and does not contain creator assets or project timeline IDs.
...
- Keep caption glyphs, progress rails and labels, information-card copy, proof marks, declared B-roll critical regions, picture-in-picture visible boxes and motion envelopes, logos, and identity marks inside the effective semantic safe region.
...
- Preserve the approved caption lane when progress moves to the top. Reflow headings, information cards, and picture-in-picture around the top band. Move or shorten a decorative outro underline before moving approved captions, and keep it visibly below the rendered caption ink.
...
- Check early, middle, late, every chapter boundary, the longest two-line caption, densest information card, picture-in-picture motion extremes, and the signature outro.
...
`layout.portrait-talking-head.safe-v1` 是仓库级、已确认的 9:16 竖屏口播联合版式参考，统一约束字幕、语义章节进度、B-roll、人物画中画和平台 UI 安全区。它不绑定个人资产，也不保存具体项目的时间线 ID。
...
- 字幕实际字形、进度轨道与标题、信息卡文案、证据标记、B-roll 关键区域、画中画可见框及完整运动范围、Logo 和身份标记必须留在有效语义安全区内。
...
- 进度条移到顶部时保留已确认字幕轨道，标题、信息卡和人物画中画绕开顶部安全带。片尾装饰下划线应先移动或缩短，并与字幕实际字形保持清晰间距，不能为了装饰线再次移动已确认字幕。
- 个人化字幕特例、身份形象和单项目碰撞修复只进入私有风格档案或项目覆盖层，不写入仓库通用基线。

### 验证要求

- 发布前同时模拟狭长手机、标准 9:16 视口和宽屏平板，并检查原始尺寸和手机尺寸。
- 检查开头、中段、结尾、每个章节边界、最长两行字幕、最密信息卡、画中画运动极值和片尾。
