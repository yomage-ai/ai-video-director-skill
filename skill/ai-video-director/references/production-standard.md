...
- Classify feedback as a reusable production rule, an explicitly shared creator style, an optional local preference, an identity adapter, or a project-only decision. The approved Xiaoxiong subtitle, loudness, color and scalable layout values are public style defaults in `xiaoxiong-public-style.json`; they are not universal platform laws. Never commit private identity media, source footage, account data or absolute media paths to this public repository.
...
- When a creator explicitly approves a color treatment, store its exact deterministic parameters and color-space assumptions in the approved public style contract when sharing was authorized, otherwise in an optional local override. The bundled Xiaoxiong natural-bright chain is the public default for comparable source conditions; original reference renders and one-video measurements remain in their source project. Recover the parameters from the command, project, or paired before/after pixels; never substitute a remembered, rounded, or visually estimated variant. Reuse the exact chain for comparable camera, lighting, exposure, white balance, input render state, and filter order. Prove the reproduction against decoded reference frames with representative Y/U/V or equivalent measurements plus visual inspection before calling it the same baseline. For materially different conditions it is a starting candidate, not an automatic look; revalidate natural skin, whites, and highlights before approval.
...
- Keep the generic RMCU behavior, parameter schema, and verification rules in the repository. A public named style may lock scalable palette, type and layout tokens; an optional identity Skill may supply an identity-marker adapter. A local override may express another creator's exception. Adapt section count, label text, durations, and fallback behavior from the approved content; change private overrides only after an explicit request or a documented accessibility failure and a new preview.
...
- Store identity-specific approved signature art and motion in the selected identity Skill; Xiaoxiong uses `xiaoxiong-ip/references/video-adapter.json`. The public editing style does not depend on that adapter. Reuse it for later videos that select the same style profile; adapt only timing to the actual speech, aspect-ratio-safe placement, and documented collision handling. A new visual concept requires another preview and approval.
...
- Prefer the most recent clean reference that matches the same content format and delivery platform. If references disagree materially, record the conflict and use a conservative speech-led social-video starting band of about `-18` to `-15 LUFS` with true peak at or below `-1 dBTP` only when no approved style target applies. The built-in Xiaoxiong target is -16.5 LUFS, acceptable -17 to -15.5 LUFS, with true peak at or below -1 dBTP. This is a starting band, not a universal platform law.
...
- Re-export the complete mix after adjustment and measure it again. Record before/after integrated LUFS, true peak, loudness range, the applied gain, the accepted reference set, and any rejected clipped outliers. Store the explicitly shared loudness target in the public style contract. Keep original audio/reference files and project measurements outside the public repository; they are not required to recover the built-in target.
