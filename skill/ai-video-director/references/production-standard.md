...
- Default to one transparent, continuous line that spans the full composition width, sits below the subtitle band, and stays close to the bottom edge with only a small inset. Separate semantic sections with proportional divider ticks and short labels; do not wrap every section in a card or leave decorative gaps unless an approved design system specifically requires them.
...
- Verify early, middle, late, and every chapter boundary in composed frames. Confirm duration-proportional widths, the correct active label, a monotonically advancing playhead, seek/re-render stability, phone-scale readability, full-width alignment, below-caption placement, and no collision with primary content.
