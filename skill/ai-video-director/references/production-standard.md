...
- Use the repository-owned `rmcu.semantic-progress.v1` contract when a reusable component is appropriate. Its landscape variant is a compact full-width top rail. Its portrait variant uses a noncritical track/playhead lane above a semantic label lane inside a platform-validated safe band. Both variants keep stable duration-proportional segment boxes.
