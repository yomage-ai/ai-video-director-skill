...
- When a private adapter uses a multi-frame personal-IP action, reuse the approved sequence and manifest order exactly. Drive the marker position, rail fill, and frame index from one normalized progress value; do not substitute an unapproved walk cycle or mirror/reorder approved frames. The personal marker must remain inside the same measured semantic-safe rail as the generic playhead.
