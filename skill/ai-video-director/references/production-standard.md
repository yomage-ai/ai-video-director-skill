...
- Bind every authored fact, number, mode label, interface state, and example to a word-level permission window. A visual may enter on or just after its first permitting spoken token, never before; it stays through the last token needed to understand the claim. Do not preload a large empty card while the speaker is still introducing the topic, and do not show the next mode while the current mode is still being named.
...
- For an explicit layout demonstration, the displayed mode is itself evidence. Switch to `A-only`, `B-only`, `B-base-A-PiP`, `A-base-B-overlay`, or another named mode only when that exact phrase is spoken; the previous mode must not anticipate the next label, and the next mode must not remain after its permitted phrase ends.
...
- Build any silhouette outline from the same alpha as the presenter, using `outlineAlpha = dilate(alpha, radius) - alpha`, and place it behind the subject. White and a width near `0.007-0.009` of canvas width are style-preview candidates, not universal defaults. An outline may soften small edge noise but may not disguise a broken matte.
- For a cutout, map the complete visible silhouette and motion envelope rather than a rectangular item frame. Bottom-left or bottom-right and intentional bottom bleed are candidates, not rules. Keep scale, anchor, side, and outline stable inside one coverage run and move them only at a semantic or layout boundary.
