...
New rough reviews use schema 5 and cover every retained interval as well as every join; new fine directions use schema 2 with information ownership and a complete dynamic presentation contract. Legacy formats remain readable with a warning, not a recommended path for new work. Use the version-bound iteration events described in [locked-master-revisions.md](references/locked-master-revisions.md); never infer creator approval or real-world quality from generated fields. / 新工程使用全保留区间听视审与完整动态样片合同，旧格式只兼容读取。每轮记录真实版本、修改原因和结果，不能把生成字段或测试通过冒充用户验收。
