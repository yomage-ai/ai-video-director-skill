...
export function checkCreatorReview(data,bound,base) {
  ...
  for(const name of ['content','picture','pauses','pace','audioLevels','decode']) check(p.checks?.[name],name);
  ...
  if(data.pauseContractVersion===1||p.pauseLedger)checkPauseLedger(p.pauseLedger,bound,base);
